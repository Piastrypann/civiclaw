...
# =========================================================
# API KEY DIRECT CONFIG (HARDCODED & SECRETS SUPPORT)
# =========================================================
...
if analyze:
    if not api_key.strip() or api_key == "PASTE_API_KEY_KAMU_DI_SINI":
        st.error("⚠️ API Key belum dimasukkan! Masukkan API Key kamu pada variabel HARDCODED_KEY di app.py.")
    elif not case_text.strip():
        st.warning("⚠️ Mohon ceritakan kronologi kejadian hukum kamu terlebih dahulu.")
    else:
        try:
            ...
            system_instruction = """
            You are CIVICLAW, an AI Legal Intelligence Agent. Analyze the user's legal situation and output absolute JSON with this exact schema:
            {
